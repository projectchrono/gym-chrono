...
# ===========================================================================================================
#
# This file contains a script to train a PPO ART agent to navigate to a goal point.
# See Environment file for more details on the environment.
...
# NOTE: this script is not directly runable as the last checkpoint is loaded and evaluated and this checkpoint
# is not provided in the repository. This script is provided for reference only.
# ============================================================================================================
...
def make_env(rank: int, seed: int = 0, mean_obstacles: int = 1) -> Callable:
    """
    ...
    def _init() -> gym.Env:
        ...
        env.set_mean_obstacles(mean_obstacles)
...
if __name__ == '__main__':
    ...
    log_path = "art_logs_lidar_4/"
    # set up logger
    new_logger = configure(log_path, ["stdout", "csv", "tensorboard"])

    # ==============================================================================================================
    # Vectorized envieroment
    # Set the mean obstacles to whatever it should be at the start of training
    # This is 1 at the very start of training but while loading a pre trained checkpoint make sure to set the
    # current mean obstacles - Note: The mean obstacles increase from 1 to a maximum of 5 over the course of training
    # based on the success rate of the agent
    # ==============================================================================================================
    mean_obstacles = 5
    env = make_vec_env(env_id=make_env(rank=0, mean_obstacles=mean_obstacles), n_envs=num_cpu,
    ...
    # ==============================================================================================================
    # Commented out is another option for the policy where the sde noise model is used. This has proven to have better
    # sim2real transfer but is slower to train
    ...
    checkpoint_dir = 'art_ppo_checkpoints_lidar_4/'
    os.makedirs(checkpoint_dir, exist_ok=True)

    # In case user wants to load from a certain checkpoint
    model = PPO.load(os.path.join(
        checkpoint_dir, f"ppo_checkpoint403"), env)
    # Replace the max range to num_of_saves ideally but the memory of my env keeps ballooning up
    success_rate_eval = 0.
    # Set if the terrain is flat
    is_flat = False
    # Set if the terrain is rigid
    is_rigid = False
    # If we want evaluation on
    model_eval = False
    eval_save_count = 5  # After how many saves do we want to evaluate the model
    epi_to_run = 10  # How many episodes to run for evaluation
    for i in range(0, 100):
        env.env_method("set_succ", success_rate_eval)
        env.env_method("set_isFlat", is_flat)
        env.env_method("set_isRigid", is_rigid)
        success_rate_eval = 0.
        ...
        # Evaluate the loaded model every 10 updates -  Note saves are made every 2 updates
        if model_eval:
            # If we have made 5 saves then we evaluate the model
            if (i % eval_save_count == 0):
                if (i != 0):  # skip evaluation for the first checkpoint
                    sucess_count = 0
                    episode_reward_list = []
                    env_single = off_road_art()
                    env_single.set_nice_vehicle_mesh()
                    env_single.set_isFlat(is_flat)
                    env_single.set_isRigid(is_rigid)
                    env_single.set_mean_obstacles(mean_obstacles)
                    ...
                    print("Sucess rate of loaded model: ",
                          sucess_count/epi_to_run)
                    ...
                    if (success_rate_eval > 0.7):
                        ...
                        f.write(f"ppo_checkpoint{i}\n")
                        f.write(
                            f"Old Mean was: {env_single.m_mean_obstacles}\n")
