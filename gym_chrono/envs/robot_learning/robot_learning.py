...
class robot_learning(ChronoBaseEnv):
    ...
    def __init__(self):
        ...
        self.bitmap_file = temp_dir  + "/utils/height_map.bmp"
        self.bitmap_file_backup =  temp_dir + "/utils/height_map_backup.bmp"
    ...
    def reset(self):
        ...
        # Create the terrain
        terrain_type = 'scm_soft_height_map'

        if 'height_map' in terrain_type:
            shape = (252, 252)
            generate_random_bitmap(shape=shape, resolutions=[(2, 2)], mappings=[(-1.5,1.5)], img_size=self.bitmap_img_size, file_name=self.bitmap_file)
        ...
        if 'rigid' in terrain_type:
            ...
            if 'flat' in terrain_type:
                self.terrain.Initialize(0,
                                        self.terrain_width,
                                        self.terrain_length,
                                        int(self.divs_per_units * self.terrain_width),
                                        int(self.divs_per_units * self.terrain_length))
            elif 'height_map' in terrain_type:
                try:
                    patch = self.terrain.Initialize(self.bitmap_file,        # heightmap file (.bmp)
                                                "test",                  # mesh name
                    ...
                    print('Corrupt Bitmap File')
                    patch = self.terrain.Initialize(self.bitmap_file_backup,        # heightmap file (.bmp)
                                                "test",                  # mesh name
                                                self.terrain_length*1.5,     # sizeX
                                                self.terrain_width*1.5,      # sizeY
                                                self.min_terrain_height, # hMin
            ...
            if 'hard' in terrain_type:
                ...
                vis_mat.SetKdTexture(chrono.GetChronoDataFile("sensor/textures/snow.jpg"))
            else:
                print(f'{terrain_type} is not allowed as a terrain type')
            self.terrain.GetMesh().material_list.push_back(vis_mat)
