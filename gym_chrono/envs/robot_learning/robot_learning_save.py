...
class robot_learning(ChronoBaseEnv):
    ...
    def __init__(self):
        ...
        if 'height_map' in self.terrain_type:
            self.out_dir = f'IKE_OUTPUT/{self.terrain_type}{self.max_terrain_height}/{self.num_obstacles}'
        else:
            self.out_dir = f'IKE_OUTPUT/{self.terrain_type}/{self.num_obstacles}'
    ...
    def reset(self):
        if self.rank != -1 and self.num_envs != -1:
            num = self.rank + self.run_number*self.num_envs
        else:
            num = self.run_number
        ...
        self.initLoc.z = self.terrain.GetHeight(chrono.ChVectorD(x, y, 0)) + 0.25
        ...
        self.vehicle.SetSuspensionVisualizationType(veh.VisualizationType_NONE)
        self.vehicle.SetSteeringVisualizationType(veh.VisualizationType_NONE)
