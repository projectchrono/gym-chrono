...
def generate_random_bitmap(shape=(256, 256), resolutions=[(8, 8)], mappings=[(-1, 1)], img_size=(100, 100), file_name="height_map.bmp", save=True, return_noise=False, initPos=None):
    ...
    if len(resolutions) != len(mappings):
        raise Exception(
            'generate_random_bitmap :: Length of shapes list and resolutions list must be the same size! Exiting...')
    elif any(s % 2 for s in shape):
        raise Exception(
            'generate_random_bitmap :: Shape must be even! Exiting...')
    ...
    if initPos is not None:
        dx = dy = 20
        x, y = map(np.array([initPos.x, -initPos.y]),
                   np.array([-40, 40]), np.array([0, shape[0]]))
        b = (slice(max(int(y - dy), 0), min(int(y + dy),
             shape[1]-1)), slice(max(int(x - dx), 0), min(int(x + dx), shape[0]-1)))
        flatten(noise, b)
...
if __name__ == '__main__':
    ...
    for _ in range(1):
        ...
        noise = generate_random_bitmap(shape=shape, resolutions=[(2, 2)], mappings=[(-1.5, 1.5)], img_size=(
            400, 400), save=save, file_name=file_name, return_noise=True, initPos=chrono.ChVectorD(1, 1, 0))
        if show:
