...
class off_road_art(ChronoBaseEnv):
    ...
    def __init__(self, additional_render_mode='None'):
        ...
        self.m_mean_obstacles = 9  # Starting mean - This is after checkpoint 150
    ...
    def reset(self, seed=None, options=None):
        ...
        art_theta = self.initialize_art_pos(seed)
        # Initialize the vehicle position -> get art_theta to set the goal position
        self.m_vehicle.SetTireRollingResistance(0.06)
        self.m_vehicle.SetMaxMotorVoltageRatio(0.16)
        self.m_vehicle.SetStallTorque(0.4)
    ...
    def get_observation(self):
        ...
        heading_diff = target_heading_to_goal - vehicle_heading
        vehicle_speed = self.m_chassis_body.GetPos_dt().Length()
        observation_array = np.array(
            [vector_to_goal_local.x, vector_to_goal_local.y, heading_diff, vehicle_speed]).astype(np.float32)
    ...
    def initialize_art_pos(self, seed):
        ...
        z = self.m_terrain.GetHeight(chrono.ChVectorD(x, y, 10.)) + 0.25
        ang = np.pi + theta
    ...
    def set_goal(self, art_theta, seed):
        ...
        # ensure that the goal is always an angle between -pi/2 and pi/2 from the art
    ...
    def update_mean_based_on_success(self):
        if self.m_success_rate_eval > 0.6:
            self.m_mean_obstacles += 1  # Increase mean if success rate is high
            print("Updated Number of obstacles mean to ", self.m_mean_obstacles)
    ...
    def add_obstacles(self, proper_collision=False):
        ...
        rock1_random = max(
            0, min(10, round(random.gauss(self.m_mean_obstacles, self.m_std_dev))))
        rock2_random = max(
            0, min(10, round(random.gauss(self.m_mean_obstacles, self.m_std_dev))))
        rock3_random = max(
            0, min(10, round(random.gauss(self.m_mean_obstacles, self.m_std_dev))))
