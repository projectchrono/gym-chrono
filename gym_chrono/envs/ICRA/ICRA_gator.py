...
class icra_gator(ChronoBaseEnv):
    ...
    def reset(self):
        ...
        if 'rigid' in self.terrain_type:
            ...
            if 'flat' in self.terrain_type:
                self.terrain.Initialize(self.terrain_length,
                                        self.terrain_width,
                                        1 / self.divs_per_units)
            elif 'height_map' in self.terrain_type:
        ...
        if 'scm' in self.terrain_type:
            self.terrain.AddMovingPatch(self.vehicle.GetChassisBody(), chrono.ChVectorD(0, 0, 0), chrono.ChVectorD(5, 5, 0))
