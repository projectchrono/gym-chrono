...
class ChronoPendulumMS(ChronoBaseEnv):
   def __init__(self):
      ...
      self.observation_space = spaces.Tuple(
          (spaces.Box(low=0, high=255, shape=(self.camera_height, self.camera_width, 3), dtype=np.uint8),
           spaces.Box(low=-200, high=200, shape=(4,), dtype=np.float)))
    ...
    # Create the set of rods in a vertical stack, along Y axis
      ...
      self.dummy_rgb = np.zeros((self.camera_height, self.camera_width, 3))
      self.reset()
   ...
   def get_ob(self):
          img_state = self.dummy_rgb
          arr_state = np.asarray([self.link_slider.GetDist(), self.link_slider.GetDist_dt(), self.pin_joint.GetRelAngle(), self.omega])
          return img_state, arr_state
